...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td_value:
        ...
        subprocess.run(["git", "config", "user.name", "v36-builder"], cwd=work, check=True)
        subprocess.run(["git", "config", "user.email", "v36-builder@example.invalid"], cwd=work, check=True)
        ...
        # Stage every change before diffing so newly-added source files are included.
        subprocess.run(["git", "add", "-A"], cwd=work, check=True)
