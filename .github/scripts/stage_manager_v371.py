...
def main() -> None:
    ...
    base_zip = repo_root / "manager/packages/FreeCam_Manager_V3.7_Source.zip"
    incoming = repo_root / "manager/incoming/v3.7.1/v37-to-v371.patch.gz.b85"
    candidate = repo_root / "manager/publish-v371.candidate.json"
    ...
    with tempfile.TemporaryDirectory() as td_value:
        ...
        subprocess.run(["git", "config", "user.name", "v371-builder"], cwd=work, check=True)
        subprocess.run(["git", "config", "user.email", "v371-builder@example.invalid"], cwd=work, check=True)
        ...
        zip_path = td / "FreeCam_Manager_V3.7_Fix1_Source.zip"
    ...
    cfg = {
        "schemaVersion": 1,
        "version": "3.7.1",
        "displayVersion": "V3.7 Fix1",
        "packageName": "FreeCam_Manager_V3.7_Fix1_Source.zip",
        "sourceSha256": zip_sha,
        "basePackageName": "FreeCam_Manager_V3.7_Source.zip",
        "patchFile": "manager/incoming/v3.7.1/v37-to-v371.patch.gz.b85",
        "patchEncoding": "base85",
        "patchCompression": "gzip",
        "patchSha256": patch_sha,
        "sourceTreeSha256": expected_tree,
        "notes": "修复开发列表后台刷新打断人工结论选择的问题：后台扫描不再清空并重建整张列表，而是按文件路径原位更新、移动或增删现有行，避免正在操作的人工结论选择器被销毁/重置。",
        "publishRevision": 1,
    }
